...
import type { IntelligenceResult, Insight, Evidence, SermonSummary } from '@/lib/backend/types';
...
function ProvenanceTag({ source }: { source?: Evidence['source'] }) {
    ...
    <ul className="space-y-1.5 border-l-2 border-border/60 pl-2">
      {evidence.map((ev, i) => (
        <li key={`evidence-${i}`} className="flex items-start justify-between gap-2">
          ...
          <ProvenanceTag source={ev.source} />
        </li>
      ))}
    </ul>
  );
...
export default function InsightsPanel() {
  ...
  const load = useCallback(async () => {
    ...
    try {
      const [insightsResult, sermons] = await Promise.all([
        backend.getSermonInsights({ sermonId: activeDocument.id, reference: activeDocument.scripture }),
        backend.listSermons(),
      ]);
  ...
  return (
    <div className="p-3 space-y-3">
      {result && !result.biblicalDataAvailable && (
