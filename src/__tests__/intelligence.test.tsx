...
import type {
  IntelligenceResult, IntelligenceInsight, IntelligenceEvidence, SermonDocument, SermonSummary,
...
function ev(partial: Partial<IntelligenceEvidence> & { label: string; value: string }): IntelligenceEvidence {
  return { kind: 'series', weight: 1, sermonIds: [], references: [], provenance: { class: 'your-archive', sourceLabel: 'Your Archive' }, ...partial };
}

function ins(partial: Partial<IntelligenceInsight> & { id: string; title: string; summary: string }): IntelligenceInsight {
...
function resultWith(insights: IntelligenceInsight[], biblicalDataAvailable = true): IntelligenceResult {
  return {
    engineVersion: 'test',
...
describe('MockSermonBackend intelligence fixtures', () => {
  it('every insight carries evidence', async () => {
    ...
    const result = await backend.getSermonInsights('sermon-001');
  ...
  it('is deterministic across repeated calls', async () => {
    const backend = new MockSermonBackend();
    const a = await backend.getSermonInsights('sermon-001');
    const b = await backend.getSermonInsights('sermon-001');
    expect(a).toEqual(b);
  });
  ...
  it('returns an empty result for the empty/insufficient sentinels', async () => {
    ...
    for (const id of ['sermon-empty', 'sermon-insufficient']) {
      const result = await backend.getSermonInsights(id);
  ...
  it('flags the missing-canon.db case and still returns archive-only insights', async () => {
    const backend = new MockSermonBackend();
    const result = await backend.getSermonInsights('sermon-no-canon');
    expect(result.biblicalDataAvailable).toBe(false);
    expect(result.insights.length).toBeGreaterThan(0);
    for (const insight of result.insights) {
      for (const e of insight.evidence) {
        expect(e.provenance.class).toBe('your-archive');
      }
    }
  });
...
describe('InsightsPanel presentation', () => {
  ...
  it('shows an evidence affordance for every insight (why-this-is-related)', async () => {
    const result = resultWith([
      ins({
        ...
        evidence: [ev({ label: 'Same series', value: 'Gospel of John', provenance: { class: 'your-archive', sourceLabel: 'Your Archive' } })],
      }),
    ]);
    const { backend } = makeInsightsBackend(result);
    resetStore(makeDoc());
  ...
  it('renders archive insights and a clear biblical-data-unavailable notice without canon.db', async () => {
    const result = resultWith(
      [ins({ id: 'a', title: 'Archive pattern', summary: 'S', evidence: [ev({ label: 'Same series', value: 'Gospel of John', provenance: { class: 'your-archive', sourceLabel: 'Your Archive' } })] })],
      false,
    );
    const { backend } = makeInsightsBackend(result);
    resetStore(makeDoc());
  ...
  it('renders provenance labels for evidence sources', async () => {
    const result = resultWith([
      ins({
        ...
        evidence: [
          ev({ label: 'Same series', value: 'Gospel of John', provenance: { class: 'your-archive', sourceLabel: 'Your Archive' } }),
          ev({ label: 'Shared reference', value: 'John 6:35', provenance: { class: 'biblical-study', sourceLabel: 'Biblical Study' }, kind: 'shared-reference', references: ['John 6:35'] }),
        ],
      }),
    ]);
    const { backend } = makeInsightsBackend(result);
